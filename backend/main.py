...
# Upload Question Paper Endpoint
@app.post("/api/upload-question-paper")
async def upload_question_paper(file: UploadFile = File(...)):
    # Validate file type
    suffix = os.path.splitext(file.filename)[1].lower()
    ...
    try:
        ...
        resp_content = response.text.strip()
        print(f"Gemini response (first 500 chars): {resp_content[:500]}")
        ...
        while i < len(lines):
            ...
            if any(keyword in line.lower() for keyword in ['total no. of pages', 'time', 'max. marks', 'answer any', 'degree examination', 'pto']):
                i += 1
                continue

            # Match question
            question_match = re.match(r'(?:(?:\d+\.|\([a-z]\)|[a-z]\.|\d+\s*[a-z]\)|Q\d+\s*[a-z]\))\s+(.+?)(?:\s*\((\d+)\s*marks?\))?$', line, re.I)
            if question_match:
                question_text = question_match.group(1).strip()
                # Check for MCQ options
                options = []
                j = i + 1
                while j < len(lines) and re.match(r'(?:[a-d]\)|[a-d]\.\s+)', lines[j], re.I):
                    option_text = re.sub(r'^[a-d]\)\s*|[a-d]\.\s*', '', lines[j]).strip()
    ...
    if not questions:
        print("No questions extracted after fallback")
        raise HTTPException(
            status_code=500,
